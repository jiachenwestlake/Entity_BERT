...
class EntityDict():
    ...
    def __init__(self, dataset, dict_path, title_vocab=None, encoding="utf-8", dict_lines=None):
        ...
        line_num = 1
        entity_list = []
        if dataset == "lm_raw_data_novel":
            with codecs.open(dict_path, 'r', encoding=encoding) as f:
                for line in tqdm(f, desc="Loading Entity Dict", total=dict_lines):
                    ...
                    line_num += 1
        elif dataset == "lm_raw_data_finance" or dataset == "lm_raw_data_thuner":
...
class Corpus(object):
    ...
    def get_iterator(self, split, batch_size, seq_len, max_doc_len, device):
        if split == "train":
            if self.dataset == "lm_raw_data_novel" or self.dataset == "lm_raw_data_finance" or self.dataset == "lm_raw_data_thuner":
